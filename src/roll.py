...
def roll_advantage(dice, score, sep_values, kwargs=None):
    ...
    :type: Dice
    :param score: score obtained on first src
    :param sep_values: each diceutils value obtained on first src
    :param kwargs:
    ...
    if kwargs["verbose"]:
        print("Second src with advantage scored:", second_roll)
...
if __name__ == '__main__':
    ...
    parser.add_argument("-o", "--open", help="reroll open dices (a diceutils is open when it scores max value)",
                        action="store_true")
    parser.add_argument("-a", "--advantage",  help="dices will be rolled twice, picking highest", action="store_true")
    parser.add_argument("-d", "--disadvantage",  help="diceutils will be rolled twice, picking lowest", action="store_true")
    ...
    if args["advantage"] and args["disadvantage"]:
        parser.error("You cant't have both options advantage and disadvantage for the same src")
